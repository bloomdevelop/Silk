...
export class Bot {
    ...
    private constructor() {
        ...
        this.eventManager = new EventManager(this.client, this.commandManager, this);
    ...
    async start(): Promise<void> {
        ...
        try {
            ...
            mainLogger.info(`AutoMod initialized in ${formatDuration(autoModTime)}`);
    ...
    async destroy(): Promise<void> {
        if (this.isDestroying) {
            mainLogger.debug('Bot destroy already in progress, skipping...');
            return;
        }
        ...
        try {
            ...
            // Close the client connection without logout
            if (this.client.websocket) {
                this.client.websocket.disconnect();
            }

            const totalTime = destroyStart();
            mainLogger.info(`Bot cleanup completed in ${formatDuration(totalTime)}`);

        } catch (error) {
            mainLogger.error('Critical error during bot cleanup:', error);
            throw error;
        }
