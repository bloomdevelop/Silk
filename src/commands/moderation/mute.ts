...
const mute: ICommand = {
    ...
    async execute(msg: Message, args: string[]): Promise<void> {
        ...
        const config = await db.getServerConfig(server._id);
        ...
        try {
            ...
            if (!targetMember) {
                await msg.reply({
                    embeds: [{
                        title: "Error",
                        description: "Cannot mute this user. Missing permissions or invalid user.",
                        colour: "#ff0000"
                    }]
                });
                return;
            ...
            // Log the mute
            this.logger?.info(`${msg.author?.username} muted ${targetMember.user?.username} for ${formatDuration(duration)}: ${reason}`);
            ...
            await msg.reply({
                embeds: [{
                    title: "Error",
                    description: "Failed to mute user. Make sure I have the required permissions.",
                    colour: "#ff0000"
                }]
            });
        }
