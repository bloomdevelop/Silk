...
const votekick: ICommand = {
    ...
    async execute(msg, args) {
                ...
                embeds: [
                    {
                        title: 'Invalid Usage',
                        description:
                            'Please provide a user ID and reason',
                        colour: '#ff0000',
                    },
                ],
            });
        ...
        try {
            ...
            if (
                serverConfig.security.blockedUsers.includes(
                    msg.author?.id ?? '',
                )
            ) {
                return msg.reply({
            ...
            let targetMember: ServerMember | undefined;
            try {
                const members =
                    await msg.channel?.server?.fetchMembers();
                targetMember = members?.members.find((member) => {
                    ...
                    return (
                        member.id.user === searchTerm || // Exact ID match
                        (username &&
                    ...
                    embeds: [
                        {
                            title: 'Error',
                            description:
                                'User not found in this server. Please provide a valid user ID or username.',
                            colour: '#ff0000',
                        },
                    ],
                });
            ...
            try {
                ...
                mainLogger.error(
                    'Failed to add vote reactions:',
                    error,
                );
                await msg.reply(
                    'Failed to start vote due to reaction error',
                );
                return;
            }
            ...
            await Promise.race([
                new Promise((resolve) =>
                    setTimeout(resolve, VOTE_DURATION),
                ),
            ...
            const message = await msg.channel?.fetchMessage(
                voteMessage.id,
            );
            if (!message)
                throw new Error('Could not fetch vote message');

            // Get reactions using Unicode
            const yesReaction = message.reactions?.get(
                encodeURIComponent('\u2705'),
            );
            const noReaction = message.reactions?.get(
                encodeURIComponent('\u274C'),
            );
            ...
            const memberCount: number | undefined = (
                await msg.channel?.server?.fetchMembers()
            )?.members.length;
            if (!memberCount)
                throw new Error('Failed to fetch member count');
            const requiredVotes = Math.ceil(
                memberCount * REQUIRED_VOTES_PERCENTAGE,
            );
            const votekickPasses =
            ...
            if (votekickPasses) {
                try {
                    ...
                    mainLogger.error(
                        'Failed to kick user after successful votekick:',
                        error,
                    );
                    return msg.reply({
                        embeds: [
                            {
                                title: 'Error',
                                description:
                                    'Votekick passed but failed to kick user. Make sure I have the required permissions.',
                                colour: '#ff0000',
                            },
                        ],
                    });
