...
async function deleteMessagesInQueue(
    messages: Message[],
): Promise<number> {
    let deletedCount = 0;
...
const purge: ICommand = {
    ...
    async execute(msg: Message, args: string[]): Promise<void> {
        ...
        const botMember = await server.fetchMember(
            msg.author?.id || '',
        );
        ...
        try {
            ...
            const messages = await RateLimitHandler.executeWithRetry(
                async () =>
                    channel.fetchMessages({ limit: amount + 1 }), // +1 to include command message
                {
                    maxRetries: 3,
                    baseDelay: 1000,
                    maxDelay: 5000,
            ...
            const targetUser = args[1]
                ? await server.fetchMember(args[1])
                : null;
            const toDelete = targetUser
                ? messages.filter(
                      (m: Message) =>
                          m.authorId === targetUser.id,
                  )
                : messages;
            ...
            const deletedCount =
                await deleteMessagesInQueue(toDelete);
                        ...
                        embeds: [
                            {
                                ...
                                description: [
                                    ...
                                    targetUser
                                        ? `from user ${targetUser.user?.username}`
                                        : '',
